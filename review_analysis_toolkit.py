...
class ReviewEvaluator:
    ...
    def get_all_attributes_score(self, df, id_column: str, review_column: str, top_attributes: List[str]) -> Dict[str, Dict[str, float | int]]:
        """
            ...
            for _, row in df.iterrows():
                ...
                if id_ in review_attribute_scores:
                    ...
                    for attribute, score in scores.items():
                        if score != 0:
                            new_data.append({'Attribute': attribute, 'Date': date, 'Score': score, 'ID': id_})
    ...
    def process_dataframe(self, df, id_column, date_column, score_column, review_column, result_path, attributes, brand_type, save_csvs=True):
        ...
        attributes_related_reviews = self.get_relevant_attributes(df, review_attribute_scores, "Text", "ID")
        attributes_summary = {}
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('../../../initial code/AImarketing/scraper/scrapers/AquaventureWaterparkReviewsTest.csv').dropna()
    print(df.columns)
    ...
    with open("../config/product_review_criteria.json") as f:
        data = json.load(f)
    attr = data["Waterpark"]
    review_evaluator.process_dataframe(df=df, review_column="Text", score_column="Score", date_column="Date", brand_type="waterpark", id_column="ID", attributes=attr, result_path="../outputs/AquaventureResult", save_csvs=True)
